function K = designLevel1ControllerILMI(A, B, C, Q, isStructured)
% DESIGNLEVEL1CONTROLLERILMI  Iterative LMI solver for Static 
% Output-Feedback (SOF) stabilization.
%
%   K = designLevel1ControllerILMI(A, B, C, Q, isStructured)
%
...
%   algorithm (i) minimises alpha (the closed-loop decay-rate margin) via
%   bisection over an LMI feasibility problem, and (ii) updates the
%   Lyapunov variable X by minimising trace(P) subject to the same LMI.
%   Convergence is declared when alpha < 0 (stabilising K found) or when
...
%   B            - (n×m) input matrix
%   C            - (p×n) output matrix
%   Q            - (n×n) positive-definite weight for the CARE initialisation
%   isStructured - logical flag:
%                    true  → K is block-diagonal PID structure  (3×9)
%                              K = [diag(Kp), diag(Ki), diag(Kd)]
%                    false → K is a full (m×p) matrix
%
% OUTPUT
...
% --------------------------------------------------------------------------
    ...
    while true
        ...
        % Step 1 — minimise alpha (bisection over LMI feasibility)
        [alpha, ~, K] = minimize_alpha(A, B, C, X, isStructured);
        fprintf('  [iLMI] alpha = %.6f\n', alpha);

        if alpha < 0
            fprintf('  [iLMI] Stabilising K found (alpha = %.6f).\n', alpha);
            break;
        end

        % Step 2 — update X by minimising trace(P)
...
% MINIMIZE_TRACE  Minimise trace(P) subject to the SOF-LMI at fixed alpha.
...
%   isStructured = false → full (m×p) matrix
    ...
    if isStructured
        sdpvar kp_wx kp_wy kp_wz
        sdpvar ki_wx ki_wy ki_wz
        sdpvar kd_wx kd_wy kd_wz
        K = [ diag([kp_wx, kp_wy, kp_wz]), ...
              diag([ki_wx, ki_wy, ki_wz]), ...
              diag([kd_wx, kd_wy, kd_wz]) ];
    else
        K = sdpvar(m, p, 'full');
    end
end
